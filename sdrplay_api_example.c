// sdrplay_api_sample_app.c : Simple console application showing the used of the
// sdrplay_api
//
...
int main(int argc, char *argv[]) {
  ...
  printf("requested Tuner%c Mode=%s\n", (reqTuner == 0) ? 'A' : 'B',
         (master_slave == 0) ? "Single_Tuner" : "Master/Slave");
    ...
    if (ndev > 0) {
      for (i = 0; i < (int)ndev; i++) {
        if (devs[i].hwVer == SDRPLAY_RSPduo_ID)
          printf("Dev%d: SerNo=%s hwVer=%d tuner=0x%.2x rspDuoMode=0x%.2x\n", i,
                 devs[i].SerNo, devs[i].hwVer, devs[i].tuner,
                 devs[i].rspDuoMode);
        else
      ...
      if (chosenDevice->hwVer == SDRPLAY_RSPduo_ID) {
        if (chosenDevice->rspDuoMode &
            sdrplay_api_RspDuoMode_Master) // If master device is available,
                                           // select device as master
        {
          ...
          if (!master_slave) // Single tuner mode
          {
            ...
            chosenDevice->rspDuoSampleFreq =
                6000000.0; // Need to specify sample frequency in master/slave
                           // mode
            printf("Dev%d: selected rspDuoMode=0x%.2x tuner=0x%.2x "
                   "rspDuoSampleFreq=%.1f\n",
                   chosenIdx, chosenDevice->rspDuoMode, chosenDevice->tuner,
      ...
      if ((err = sdrplay_api_SelectDevice(chosenDevice)) !=
          sdrplay_api_Success) {
      ...
      if ((err = sdrplay_api_GetDeviceParams(
               chosenDevice->dev, &deviceParams)) != sdrplay_api_Success) {
      ...
      if (deviceParams == NULL) {
        printf(
            "sdrplay_api_GetDeviceParams returned NULL deviceParams pointer\n");
        goto CloseApi;
      }
      ...
      if ((err = sdrplay_api_Init(chosenDevice->dev, &cbFns, NULL)) !=
        ...
        if (err ==
            sdrplay_api_StartPending) // This can happen if we're starting in
                                      // master/slave mode as a slave and the
      ...
      while (1) // Small loop allowing user to control gain reduction in +/-1dB
                // steps using keyboard keys
      {
        if (kbhit()) {
          c = getch();
          if (c == 'q') {
